...
fn hjkl_moves() {
    ...
    // Move right with l
    let (c, cmds) = eng.handle_event(&buf, cur, key('l'));
    cur = c;
    ...
    // Move down with j
    let (c, cmds) = eng.handle_event(&buf, cur, key('j'));
    cur = c;
    assert_eq!(cur, Position { line: 1, col: 1 });
    assert_eq!(cmds.len(), 1);

    // Move left with h
    let (c, cmds) = eng.handle_event(&buf, cur, key('h'));
    cur = c;
    assert_eq!(cur, Position { line: 1, col: 0 });
    assert_eq!(cmds.len(), 1);

    // Move up with k
    let (c, cmds) = eng.handle_event(&buf, cur, key('k'));
    cur = c;
...
fn zero_and_dollar() {
    ...
    let (c, cmds) = eng.handle_event(&buf, cur, key('0'));
...
fn g_and_big_g() {
    ...
    // G goes to last line
    let (c, cmds) = eng.handle_event(&buf, cur, key('G'));
    assert_eq!(c.line, 3); // 0-indexed, so line 4 is index 3
    assert_eq!(c.col, 0);
    assert_eq!(cmds.len(), 1);

    // gg goes to first line
    let (c, _) = eng.handle_event(&buf, c, key('g'));
    assert_eq!(c.line, 3); // no change yet, g is pending
    let (c, cmds) = eng.handle_event(&buf, c, key('g'));
    assert_eq!(c.line, 0);
    assert_eq!(c.col, 0);
    assert_eq!(cmds.len(), 1);
}
...
fn counts_with_movements() {
    ...
    // 3l moves right 3
    let (_, _) = eng.handle_event(&buf, cur, key('3'));
    let (c, cmds) = eng.handle_event(&buf, cur, key('l'));
    ...
    // 2j moves down 2
    let (_, _) = eng.handle_event(&buf, cur, key('2'));
    let (c, cmds) = eng.handle_event(&buf, cur, key('j'));
...
fn count_with_g_motions() {
    ...
    let (_, _) = eng.handle_event(&buf, c, key('2'));
    let (_, _) = eng.handle_event(&buf, c, key('g'));
    let (c, cmds) = eng.handle_event(&buf, c, key('g'));
    assert_eq!(c.line, 1);
    assert_eq!(cmds.len(), 1);
}
...
fn insert_mode_transitions() {
    ...
    let (c, cmds) = eng.handle_event(&buf, Position { line: 0, col: 5 }, key('A'));
...
fn insert_mode_text_input() {
    ...
    let (c, _) = eng.handle_event(&buf, cur, key('i'));

    // Type 'x'
    let (c, cmds) = eng.handle_event(&buf, c, InputEvent::ReceivedChar('x'));
...
fn zero_as_motion_vs_count() {
    ...
    let cur = Position { line: 0, col: 0 };
    let (_, _) = eng.handle_event(&buf, cur, key('1'));
    let (_, _) = eng.handle_event(&buf, cur, key('0'));
    let (c, cmds) = eng.handle_event(&buf, cur, key('l'));
...
fn edge_cases() {
    ...
    let (c, _) = eng.handle_event(&buf, cur, key('l'));
    assert_eq!(c, cur); // can't move right on empty line

    // Moving up/down preserves column preference
    let cur = Position { line: 0, col: 0 };
    let (c, _) = eng.handle_event(&buf, cur, key('j')); // to empty line
    assert_eq!(c, Position { line: 1, col: 0 });
    let (c, _) = eng.handle_event(&buf, c, key('j')); // to 'y' line
